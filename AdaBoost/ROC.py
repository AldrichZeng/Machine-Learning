...
def plotROC(predStrengths, classLabels):
    ...
    for index in sortedIndicies.tolist()[0]:
        if classLabels[index] == 1.0:#真正例，要减少Y值，降低True Positive Rate
            delX = 0;
            delY = yStep
        else:# 假正例
            delX = xStep;
            delY = 0
            ySum += cur[1]  # 高度累加
        ax.plot([cur[0], cur[0] - delX], [cur[1], cur[1] - delY], c='b')  # 绘制线段
        cur = (cur[0] - delX, cur[1] - delY)  # 更新绘制光标的位置
    ax.plot([0, 1], [0, 1], 'b--')# 随机猜测的ROC曲线
    plt.title('ROC curve for AdaBoost Horse Colic Detection System')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    ax.axis([0, 1, 0, 1])  # 横纵坐标的范围
...
if __name__ == '__main__':
    ...
    _, aggClassEst = adaBoostTrainDS(dataArr, LabelArr, 50)
