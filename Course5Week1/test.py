import numpy as np
import Course5Week1.cllm_utils as cllm_utils
# 获取名称
...
ix_to_char = {i:ch for i, ch in enumerate(sorted(chars))}  # 将每个索引映射回相应的字符字符
print(char_to_ix)
print(ix_to_char)
...
def clip(gradients, maxValue):
    ...
    for gradient in [dWaa, dWax, dWya, db, dby]:
        np.clip(gradient, -maxValue, maxValue, out=gradient)  # 梯度向量的每一个元素都被限制在[−N，N] [-N，N][−N，N]的范围
...
def sample(parameters, char_to_is, seed):
    ...
    vocab_size = by.shape[0]  #vocabulary size
    n_a = Waa.shape[1]
    ...
    counter = 0  #
    newline_character = char_to_ix["\n"]  # newline_character值为0

    while (idx != newline_character and counter < 50):# 直到得到一个“\n”字符，表明已经到达恐龙名称的末尾
        # 步骤2：使用公式1、2、3进行前向传播
        ...
        # 步骤3：从概率分布y中抽取词汇表中 字符的索引
        idx = np.random.choice(list(range(vocab_size)), p=y.ravel())

        # 添加到索引中
        indices.append(idx)

        # 步骤4:将输入字符重写为与索引对应的 字符。
        x = np.zeros((vocab_size, 1))
        x[idx] = 1
