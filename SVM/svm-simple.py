...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    labelMat = np.mat(classLabels).transpose()  # 转换为numpy的mat存储

    m, n = np.shape(dataMatrix)  # 统计dataMatrix的维度
    ...
    while (iter_num < maxIter):  # 最多迭代matIter次
        ...
        for i in range(m):
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                ''' 步骤1：计算误差alpha_j的误差Ej '''
                fXj = float(np.multiply(alphas, labelMat).T * (dataMatrix * dataMatrix[j, :].T)) + b
                Ej = fXj - float(labelMat[j])
                alphaIold = alphas[i].copy();# 保存更新前的aplpha值，使用深拷贝
                alphaJold = alphas[j].copy();# 保存更新前的aplpha值，使用深拷贝
                ''' 步骤2：计算上下界L和H '''
